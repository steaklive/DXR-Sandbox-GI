...
void CSMain(uint3 Gid : SV_GroupID, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    [unroll]
    for (int i = 0; i < 8; i++)
        values[i] = voxelTextureSrcPosX.Load(int4(sourcePos + anisoOffsets[i], MipLevel));
    ...
    [unroll]
    for (int i = 0; i < 8; i++)
        values[i] = voxelTextureSrcNegX.Load(int4(sourcePos + anisoOffsets[i], MipLevel));
    ...
    [unroll]
    for (int i = 0; i < 8; i++)
        values[i] = voxelTextureSrcPosY.Load(int4(sourcePos + anisoOffsets[i], MipLevel));
    ...
    [unroll]
    for (int i = 0; i < 8; i++)
        values[i] = voxelTextureSrcNegY.Load(int4(sourcePos + anisoOffsets[i], MipLevel));
    ...
    [unroll]
    for (int i = 0; i < 8; i++)
        values[i] = voxelTextureSrcPosZ.Load(int4(sourcePos + anisoOffsets[i], MipLevel));
    ...
    [unroll]
    for (int i = 0; i < 8; i++)
        values[i] = voxelTextureSrcNegZ.Load(int4(sourcePos + anisoOffsets[i], MipLevel));
